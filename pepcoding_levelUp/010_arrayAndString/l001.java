class l001 {
    ...
    // max sum in all rotation of array
...
// 76 min window containing target string
...
// min window containing all self character
...
// longest subarray sum divisible by k gfg
...
// maximum length containing equal number of 0 and  1
...
// count length containg equal no of 0 and 1
...
// ======================================== 904 -> for you
 public int totalFruit(int[] tree) {
        ...
        while(ei < n){
            if(freq[tree[ei++]]++ == 0) disCount++;
            
            while( disCount > 2){
                if(freq[tree[si++]]-- == 1) disCount--;
            }
            len = Math.max(len, ei - si);
...
// kadane algo to find array with max sum
...
// 1074 of submatrix that sum to target
...
// 363- max sum of rectangle sum not larger than k
